...
librarian::shelf(RPostgres, DBI, rstudioapi, dplyr, tidyr, readr, ggplot2, lubridate, magrittr, here, viridis) # remove ggplot when plotting moved to different script
...
# Create capture history that includes only frogs relocated in 2021 (longer capture history)
# NOTE: bd_load data from 2022 is currently missing from amphibians database. Create figure when 2022 bd_load data are added
cmr_captures2 <- relocate_all %>% 
  filter(site_id != 70279 &
           release_date < "2022-01-01") %>% # restrict data to relocations conducted in 2021
  distinct(pit_tag_ref) %>% 
  inner_join(cmr_captures, by = "pit_tag_ref") %>% 
  mutate(site_id = as.character(site_id), 
         site_id = recode(site_id, "70279" = "70413", "72093" = "70413"))

# Assign periods to survey dates  
...
# Join periods to capture history
cmr_captures2 <- cmr_captures2 %>% 
